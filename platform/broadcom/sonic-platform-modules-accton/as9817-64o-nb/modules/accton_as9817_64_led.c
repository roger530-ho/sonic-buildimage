/*
 * A LED driver for the Accton as9817_64 led
 *
...
static const struct led_reg led_reg_map[] = {
    /*  Type             Mode             R               G             B      */
    {LED_TYPE_DIAG,  LED_MODE_RED,   {{0x8E, 0xFF}, {0x8D, 0x00}, {0x8C, 0x00}}, },
    {LED_TYPE_DIAG,  LED_MODE_GREEN, {{0x8E, 0x00}, {0x8D, 0xFF}, {0x8C, 0x00}}, },
    {LED_TYPE_DIAG,  LED_MODE_OFF,   {{0x8E, 0x00}, {0x8D, 0x00}, {0x8C, 0x00}}, },

    {LED_TYPE_LOC,   LED_MODE_BLUE_BLINKING, {{0x88, 0x00}, {0x87, 0x00}, {0x86, 0xFF}}, },
    {LED_TYPE_LOC,   LED_MODE_OFF,   {{0x88, 0x00}, {0x87, 0x00}, {0x86, 0x00}}, },

    {LED_TYPE_FAN,   LED_MODE_RED,   {{0x8B, 0xFF}, {0x8A, 0x00}, {0x89, 0x00}}, },
    {LED_TYPE_FAN,   LED_MODE_GREEN, {{0x8B, 0x00}, {0x8A, 0xFF}, {0x89, 0x00}}, },
    {LED_TYPE_FAN,   LED_MODE_OFF,   {{0x8B, 0x00}, {0x8A, 0x00}, {0x89, 0x00}}, },

    {LED_TYPE_ALARM, LED_MODE_RED,   {{0x91, 0xFF}, {0x90, 0x00}, {0x8F, 0x00}}, },
    {LED_TYPE_ALARM, LED_MODE_OFF,   {{0x91, 0x00}, {0x90, 0x00}, {0x8F, 0x00}}, },


    {LED_TYPE_PSU1,  LED_MODE_RED,   {{0x82, 0xFF}, {0x81, 0x00}, {0x80, 0x00}}, },
    {LED_TYPE_PSU1,  LED_MODE_GREEN, {{0x82, 0x00}, {0x81, 0xFF}, {0x80, 0x00}}, },
    {LED_TYPE_PSU1,  LED_MODE_OFF,   {{0x82, 0x00}, {0x81, 0x00}, {0x80, 0x00}}, },


    {LED_TYPE_PSU2,  LED_MODE_RED,   {{0x85, 0xFF}, {0x84, 0x00}, {0x83, 0x00}}, },
    {LED_TYPE_PSU2,  LED_MODE_GREEN, {{0x85, 0x00}, {0x84, 0xFF}, {0x83, 0x00}}, },
    {LED_TYPE_PSU2,  LED_MODE_OFF,   {{0x85, 0x00}, {0x84, 0x00}, {0x83, 0x00}}, },
};
...
struct led_blinking_type_info {
    enum led_type type;
    enum led_light_mode  mode;
    struct blinking_status blinking[3];
};
 ...
 */
static enum led_light_mode as9817_64_led_blinking(enum led_type type, enum led_light_mode mode)
{
    ...
    for (i = 0; i < ARRAY_SIZE(led_blinking_type_table); i++) {
        ...
        for (j = 0; j < ARRAY_SIZE(info->blinking); j++) {
            ...
            val = as9817_64_led_read_value(info->blinking[j].reg_addr);
...
static void as9817_64_led_set(enum led_type type, enum led_light_mode mode)
{
    ...
    for (i = 0; i < ARRAY_SIZE(led_reg_map); i++) {
        ...
        for (j = 0; j < ARRAY_SIZE(led_reg_map[i].rgb); j++) {
            as9817_64_led_write_value(led_reg_map[i].rgb[j].reg_addr, led_reg_map[i].rgb[j].reg_val);
        }
    }
...
static enum led_brightness as9817_64_led_get(enum led_type type)
{
    ...
    for (i = 0; i < ARRAY_SIZE(led_reg_map); i++) {
        ...
        if (match == 3) { /* if match R, G , B registr value */
...
#if 0
static void as9817_64_led_auto_set(struct led_classdev *led_cdev,
                                   enum led_brightness led_light_mode)
{
}

static enum led_brightness as9817_64_led_auto_get(struct led_classdev *cdev)
{
...
static const struct platform_device_id as9817_64_led_id[] = {
    { DRVNAME, 0 },
    {}
};
MODULE_DEVICE_TABLE(platform, as9817_64_led_id);
...
module_platform_driver(as9817_64_led_driver);

MODULE_AUTHOR("Roger Ho <roger530_ho@accton.com>");
MODULE_DESCRIPTION("AS9817-64-NB LED driver");
MODULE_LICENSE("GPL");
