...
static struct as9817_64_psu_data *as9817_64_psu_update_device(struct i2c_client *client);
...
static ssize_t set_power_cycle(struct device *dev, struct device_attribute *da,
                               const char *buf, size_t count);
static int as9817_64_psu_write_word(struct i2c_client *client, u8 reg, u16 value);
...
static SENSOR_DEVICE_ATTR(psu_mfr_fan_speed_min,   S_IRUGO, show_linear, NULL, PSU_MFR_FAN_SPEED_MIN);
static SENSOR_DEVICE_ATTR(psu_mfr_fan_speed_max,   S_IRUGO, show_linear, NULL, PSU_MFR_FAN_SPEED_MAX);
static SENSOR_DEVICE_ATTR(psu_v_out_min,   S_IRUGO, show_threshold, NULL, PSU_VOUT_WARN_LO);
static SENSOR_DEVICE_ATTR(psu_v_out_max,   S_IRUGO, show_threshold, NULL, PSU_VOUT_WARN_HI);
static SENSOR_DEVICE_ATTR(psu_p_out_max,   S_IRUGO, show_threshold, NULL, PSU_POUT_WARN_HI);
static SENSOR_DEVICE_ATTR(psu_p_out_crit,   S_IRUGO, show_threshold, NULL, PSU_POUT_CRIT_HI);
static SENSOR_DEVICE_ATTR(psu_temp3_max,   S_IRUGO, show_threshold, NULL, PSU_TEMP3_WARN_HI);
static SENSOR_DEVICE_ATTR(psu_temp3_crit,   S_IRUGO, show_threshold, NULL, PSU_TEMP3_CRIT_HI);
static SENSOR_DEVICE_ATTR(psu_power_cycle,   S_IWUSR, NULL, set_power_cycle, PSU_POWER_CYCLE);
                                  ...
                                  const char *buf, size_t count)
{
    ...
    as9817_64_psu_write_word(data->client, 0x3B + nr, data->reg_val.fan_duty_cycle[nr]);
    mutex_unlock(&data->update_lock);
                               ...
                               const char *buf, size_t count)
{
    ...
    mutex_lock(&data->update_lock);
    as9817_64_psu_write_word(data->client, 0xEE, enable);
    mutex_unlock(&data->update_lock);
...
static ssize_t show_threshold(struct device *dev, struct device_attribute *da, char *buf)
{
    struct sensor_device_attribute *attr = to_sensor_dev_attr(da);
    struct as9817_64_psu_data *data = dev_get_drvdata(dev);
    int64_t *val;
                               ...
                               const struct i2c_device_id *dev_id)
{
    ...
    data->hwmon_dev = hwmon_device_register_with_groups(&client->dev, client->name, data,  as9817_64_psu_groups);
...
static void as9817_64_psu_remove(struct i2c_client *client)
{
...
static int as9817_64_psu_read_block(struct i2c_client *client, u8 command, u8 *data,
                                    int data_len)
{
    int status = 0, retry = I2C_RW_RETRY_COUNT;
                                       ...
                                       struct pmbus_register_value *data)
{
    ...
    dev_dbg(&client->dev, "Starting as9817-64 PSU update\n");
    ...
    if (pmbus_register_supported(driver_data->chip, command)) {
        ...
        if( fan_ptr != NULL )
        {
            ...
            status = as9817_64_psu_read_block(client, command, fan_dir, ARRAY_SIZE(fan_dir)-1);
            if (status < 0) {
                dev_dbg(&client->dev, "reg 0x%02x, err %d\n", command, status);
                goto exit;
            }
...
{
    return i2c_add_driver(&as9817_64_psu_driver);
}

static void __exit as9817_64_psu_exit(void)
{
    i2c_del_driver(&as9817_64_psu_driver);
}
...
MODULE_DESCRIPTION("AS9817-64-NB PSU driver");
MODULE_LICENSE("GPL");
