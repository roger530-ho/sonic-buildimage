...
static struct as9817_64_fan_data *as9817_64_fan_update_device(struct device *dev);
...
DECLARE_FAN_SENSOR_DEVICE_ATTR(1, 11);
DECLARE_FAN_SENSOR_DEVICE_ATTR(2, 12);
DECLARE_FAN_SENSOR_DEVICE_ATTR(3, 13);
DECLARE_FAN_SENSOR_DEVICE_ATTR(4, 14);
...
static struct attribute *as9817_64_fan_attributes[] = {
    ...
    DECLARE_FAN_ATTR(1, 11),
    DECLARE_FAN_ATTR(2, 12),
    DECLARE_FAN_ATTR(3, 13),
    DECLARE_FAN_ATTR(4, 14),
...
static int as9817_64_fan_write_value(struct i2c_client *client, u8 reg, u8 value)
{
    int status = 0, retry = I2C_RW_RETRY_COUNT;
                              ...
                              const char *buf, size_t count)
{
    ...
    switch (attr->index) {
        case FAN1_PWM ... FAN4_PWM:
            ...
            /* Front FAN */
            as9817_64_fan_write_value(data->client, fan_reg[FAN1_FRONT_PWM_REG + idx], reg_val);
            /* Rear FAN */
            as9817_64_fan_write_value(data->client, fan_reg[FAN1_REAR_PWM_REG + idx], reg_val);
                              ...
                              const char *buf, size_t count)
{
    ...
    switch (attr->index) {
        case FAN1_LED ... FAN4_LED:
            ...
            mutex_lock(&data->update_lock);
            as9817_64_fan_write_value(data->client, fan_reg[FAN_LED_REG], reg_val);
            /* force update register */
            data->valid = 0;
            mutex_unlock(&data->update_lock);
            break;
                              ...
                              char *buf)
{
    ...
    switch (attr->index) {
        ...
        case FAN1_PWM ... FAN4_PWM:
            ...
            reg_val = data->reg_val[FAN1_FRONT_PWM_REG + ((attr->index - FAN1_PWM) * 2)] & 0x0F;
            //ret = sprintf(buf, "%u\n", DIV_ROUND_CLOSEST(reg_val * 666, 100));
            ret = sprintf(buf, "%u\n", (reg_val * 667) / 100);
            break;
        ...
        case FAN1_DIRECTION ... FAN4_DIRECTION:
            ret = sprintf(buf, "%d\n",
                          reg_val_to_direction(attr->index - FAN1_DIRECTION));
            break;
        case FAN1_FRONT_TARGET_SPEED_RPM ... FAN4_FRONT_TARGET_SPEED_RPM:
            idx = FAN1_FRONT_PWM_REG + ((attr->index - FAN1_FRONT_TARGET_SPEED_RPM) * 2);
            ret = sprintf(buf, "%d\n",
                          front_fan_target_speed_f2b[data->reg_val[idx]]);
            break;
        case FAN1_REAR_TARGET_SPEED_RPM ... FAN4_REAR_TARGET_SPEED_RPM:
            idx = FAN1_REAR_PWM_REG + ((attr->index - FAN1_REAR_TARGET_SPEED_RPM) * 2);
            ret = sprintf(buf, "%d\n",
                                ...
                                const struct i2c_device_id *dev_id)
{
    ...
    data->hwmon_dev = hwmon_device_register_with_groups(&client->dev, client->name, data, as9817_64_fan_groups);
...
static void as9817_64_fan_remove(struct i2c_client *client)
{
