...
DEBUG = False
FAN_SYSLED_FNODE= "/sys/class/leds/as9817_64_led::fan/brightness"
SYSLED_MODES = {
...
class device_monitor(object):
    ...
    def power_off_dut(self):
        ...
        # Power Cycle
        status = self.glob_write_txt_file("/sys/bus/i2c/devices/77-0058/hwmon/hwmon*/psu_power_cycle", "1")
        if status == FALSE:
            logging.warning("Fail to power cycle PSU1!")
        status = self.glob_write_txt_file("/sys/bus/i2c/devices/77-0059/hwmon/hwmon*/psu_power_cycle", "1")
        if status == FALSE:
            logging.warning("Fail to power cycle PSU2!")
    ...
    def reset_mac(self):
        ...
        cmd_str = "echo 1 > /sys/bus/i2c/devices/0-0060/reset_mac"
        status, output = getstatusoutput_noshell(cmd_str.split())
        if status != 0:
            logging.error("Fail to rest MAC.[{}]".format(output))
    ...
    def set_fan_sysled(self, color):
        ...
        if mode is None:
            ...
            return self.glob_write_txt_file(FAN_SYSLED_FNODE, mode)
    ...
    def manage_fans(self):
        ...
        if len(fan_fail_list) == 0:
            self.set_fan_sysled('STATUS_LED_COLOR_GREEN')
            new_duty_cycle = FAN_POLICY[current_state][0]
        else:
            self.set_fan_sysled('STATUS_LED_COLOR_RED')
